...
namespace EmployeePayrollADO
{
    class ER
    {
        public static string connection = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=paroll_service;Integrated Security=True";
        ...
        public int RetrieveAllData()
        {
            int count = 0;
            sqlConnection.Open();
            string query = @"Select CompanyID,CompanyName, EmployeeID,EmployeeName,EmployeeAddress,EmployeePhoneNum,StartDate,Gender, BasicPay,TaxablePay,IncomeTax,NetPay,Deductions,DepartmentId,DepartName
from Company inner join Employee on Company.CompanyID = Employee.Company_Id inner join PayRollCalculate on PayRollCalculate.Employee_Id = Employee.EmployeeId
inner join EmployeeDept on EmployeeDept.Employee_Id = Employee.EmployeeID inner join DepartmentTable on DepartmentTable.DepartmentId = EmployeeDept.Dept_Id";
        ...
        {
            sqlConnection.Open();
            string query = @"update PayrollCalculate set BasicPay = 3000000 where Employee_Id = (Select EmployeeID from Employee where EmployeeName = 'niles')";
        ...
        public string PerformAlgebricFunctions(string Gender)
        {
            string result = null;
